...
FilesSelectionWizardPage::FilesSelectionWizardPage(LedaProjectWizardDialog *ledaProjectWizard, QWidget *parent)
    : QWizardPage(parent), m_ledaProjectWizardDialog(ledaProjectWizard), m_model(0), m_finished(false)
{
...
void FilesSelectionWizardPage::initializePage()
{
    ...
    m_model->startParsing(m_ledaProjectWizardDialog->path());
...
void FilesSelectionWizardPage::parsingFinished()
{
    ...
    applyFilter();
    // work around qt
    m_ledaProjectWizardDialog->setTitleFormat(m_ledaProjectWizardDialog->titleFormat());
}
